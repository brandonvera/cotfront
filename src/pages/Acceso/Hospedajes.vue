          ...
          <card class="card-plain">
            <div class="table table-responsive-xl">
              <table class="table table-hover table-dark">
                ...
                <tr v-for="(item, index) in paginated('hosp')" :key="index">
                  <slot> 
                    <td>{{item.id}}</td>
		...
		methods: {
	      verHospedajes() {
	        axios.get(`http://127.0.0.1:8000/api/auth/hospedaje/${this.municipio}`,{
              ...
              if(this.rol == 1) {
                this.hosp = result.data.hospedajeTodo;
              } else {
                this.hosp = result.data.hospedaje;
              }	            
	          }).catch(error => {
	            console.log(error);
	        })
	      ...
	      buscarHospedajes() {
	        ...
	        this.setTimeoutBuscador = setTimeout(this.verHospedajes, 360)
	      },

	      async crear() {
	        this.$router.push('crear-hospedajes');
	      },
	      ...
	      async editar(id) {
	        localStorage.setItem("hos", id);
	        this.$router.push('editar-hospedajes');
	      },
	      ...
	      async exportar() {
	        ...
	        .then((response) => {
	          ...
	          fileLink.setAttribute('download', `Hospedajes.xlsx`);
