          ...
          <card class="card-plain">
            <div class="table table-responsive-xl">
              <table class="table table-hover table-dark">
                ...
                <tr v-for="(item, index) in paginated('natu')" :key="index">
                  <slot> 
                    <td>{{item.id}}</td>
                    <td>{{item.nombre}}</td>
...
<script>
	...
	export default {
      ...
      buscarNaturales() {
        ...
        this.setTimeoutBuscador = setTimeout(this.verNaturales, 360)
      },

      async crear() {
        this.$router.push('crear-naturales');
      },
      ...
      async editar(id) {
        localStorage.setItem("nat", id);
        this.$router.push('editar-naturales');
      },
      ...
      async eliminar(id) {
        try 
        {
          ...
          if (response.status == 200) {
            this.$swal({
                  icon: 'success',
                  title: 'Atractivo Natural desactivado con exito!',
                  text: 'El atractivo natural seleccionado fue desactivado con exito!',
            })
            this.$router.push('/opciones')
          }
        }
      ...
      async exportar() {
        ...
        .then((response) => {
          ...
          fileLink.setAttribute('download', `AtractivosNaturales.xlsx`);
      ...
      importar() {
        ...
        .then((response) => {
          this.$swal({
            icon: 'success',
            title: 'Atractivo Natural importado con exito!',
            text: 'El atractivo natural seleccionado fue importado con exito!',
          })
        })
        .catch((error) => {
          this.$swal({
